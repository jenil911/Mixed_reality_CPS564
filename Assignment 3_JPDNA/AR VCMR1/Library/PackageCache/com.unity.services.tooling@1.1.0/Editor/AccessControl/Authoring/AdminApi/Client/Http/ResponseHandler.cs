...
namespace Unity.Services.Tooling.Editor.AccessControl.Authoring.Client.Http
{
    /// <summary>
    ...
    internal static class ResponseHandler
    {
        ...
        private static void PatchActualError(object httpError, Type errorType, string errorTitle, string errorDetail)
        {
            if (httpError is null || errorType is null) return;
            if (httpError.GetType() != errorType) return;
            var titleProp = errorType.GetProperty("Title");
            if (titleProp is not null && titleProp.CanWrite && string.IsNullOrEmpty(titleProp.GetValue(httpError) as string))
            {
                titleProp.SetValue(httpError, errorTitle);
            }
            var detailProp = errorType.GetProperty("Detail");
            if (detailProp is not null && detailProp.CanWrite && string.IsNullOrEmpty(detailProp.GetValue(httpError) as string))
            {
                using (var stringReader = new StringReader(errorDetail))
                using (var stringWriter = new StringWriter())
                {
                    ...
                    try
                    {
                        ...
                        errorDetail = stringWriter.ToString();
                    }
                    finally
                    {
                        detailProp.SetValue(httpError, errorDetail);
                    }
        ...
        private static HttpException CreateHttpException(HttpClientResponse response, Type responseType)
        {
            ...
            try
            {
                ...
                // temporary fix because of a mismatch between the specs and the api
                PatchActualError(dataObject, responseType, response.ErrorMessage, Encoding.UTF8.GetString(response.Data ?? new byte[0]));
